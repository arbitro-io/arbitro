...
async fn setup(
    ...
    // `*.>` , not a bare `>`: catalog admission requires a stream to own a
    // slice rather than claim everything, so `>` now returns
    // InvalidStreamFilter. Covers `ack.bench.topic` either way.
    let resp = client
