...
| P2 | `src/cluster/storage.rs:824-835` + `src/cluster/apply_loop.rs:96-101` | `read_entries` pushes `LogEntry<'a>` views into `out` *before* returning `Err("payload_buf too small")` (partial fill at storage.rs:826-841). `apply_loop` then calls `payload_buf.resize(...)` (apply_loop.rs:100-101) while the partially-filled `entries` Vec still holds transmuted `&'a` references into the old allocation. Never dereferenced afterwards (fresh `Vec::new()` per iteration, apply_loop.rs:95), so no observable crash today, but it is invalidated-reference UB per aliasing rules and one refactor away from a real use-after-free. Fix: `out.clear()` on the error path. |
| P2 | `src/persistence/command_log.rs:124,143` | Replay allocates `vec![0u8; len]` from an untrusted length prefix (up to 4 GiB) before reading. A corrupted/hostile command-log file can OOM the broker at boot. Cap `len` (metadata commands are small). |
| P2 | `src/transport/dispatch_v2.rs:586-608` vs `:651-660` | Batch publish: msg-ids are correctly extracted from HAS_HEADERS `ExtendedPayload` for dedup (586-608), but the stored entries are written with `flags: 0` (657). Consumers then receive the raw ExtendedPayload TLV bytes as payload (drain unwraps only when `HAS_HEADERS` is set, `src/shard/drain.rs:787-800`), and restart dedup rebuild misses these ids (`src/persistence/recovery.rs:432`). Latent: the reference client only sets HAS_HEADERS on single publish (`arbitro-client-tokio/src/publish/mod.rs:262-305`), so only reachable by foreign clients today. |
...
| P2 | `src/shard/worker.rs:277-284` | **Busy-poll under sustained backpressure.** When a consumer is capacity-blocked or its writer channel full, `more_pending` keeps the gate open and the stalled drain loops with only a 50 µs sleep → ~20k cycles/s/shard of CPU indefinitely (known open follow-up). |
...
| Sev | Location | Finding / failure scenario |
|-----|----------|----------------------------|
| P1 | `src/shard/drain.rs:364-372, 397-400` vs `src/transport/registry.rs:454-467` | **Asymmetric slow-consumer policy: delivery path never ejects.** The reply path closes a connection whose outbound queue is full (ROB-22, registry.rs:460-466), but the drain treats a full writer channel as transient forever (`Backpressured` → retry next cycle). A live-but-not-reading consumer (keeps pinging, never drains) pins the shard cursor at its first undeliverable seq indefinitely → busy-poll (2.2) + rewalk-duplicates to healthy consumers each cycle. No backpressure-duration bound exists. |
| P2 | `src/shard/worker.rs:458-462` + `src/shard/handlers.rs:325-335`, absent in `handlers.rs:708-743` | `dlq_nack_counts` entries are removed on ack (handlers.rs:252) and on threshold, but never on consumer delete/disconnect → unbounded growth under nack-heavy churn when `max_nack > 0` (currently opt-in, so bounded exposure). |
| P2 | `src/cluster/state_machine.rs:72-74, 441` | `applied: Vec<ClusterCommand>` retains every committed metadata command forever; `snapshot()` (445-448) serializes the full history. Unbounded RAM + ever-growing snapshots on long-lived clusters. |
...
| P2 | `src/transport/dispatch_v2.rs:679-768` | Delayed publish (delay > 0) bypasses both the idempotency check and the quota pre-check that the immediate path enforces — a dedup window or discard=1 quota does not apply to delayed messages. |
...
| Sev | Location | Finding / failure scenario |
|-----|----------|----------------------------|
| P1 | (same as 2.3-P1) `src/shard/drain.rs:397-400` + `src/shard/worker.rs:277-284` | A single slow/dead-reading consumer stalls its shard's cursor and burns CPU; no eviction, no bound. This is the primary DoS surface for an authenticated client. |
...
| I4 | Dedup + clean payload for **batch** publish with headers | **GAP** | Server stores batch entries with `flags: 0` (`dispatch_v2.rs:651-660`) — delivered payload would contain the TLV wrapper and restart-rebuild misses the ids. Proposed: `batch_publish_with_headers_roundtrip_and_restart_dedup` (needs raw-frame client or client support). Rank: 7. |
...
| O2 | Order preserved across redelivery/rewind | PARTIAL | Only the monotonic-happy-path is asserted; redelivered messages re-enter in store order by construction (`drain_read` walk, `drain.rs:233-255`) but no test pins ordering after a mid-stream nack. Low risk. |
| U1 | Fsync policy actually applied to the message journal; kill -9 durability | **GAP / UNVERIFIED** | Fix verified in code (`router.rs:191-213`, SEC-8) but no e2e kills the process non-gracefully under `ARBITRO_FSYNC_POLICY=every` and asserts the tail batch survives. All restart tests use graceful shutdown. Proposed: `fsync_every_survives_process_kill` (spawn broker as child process, SIGKILL after publish_wait RepOk, restart, assert message present). Rank: **5**. |
...
| C4 | Same durable NAME subscribed from two connections — defined semantics (shared consumer identity) | **GAP** | `get_or_create_consumer` dedups by name (`dispatch_v2.rs:1636`) so both connections bind the same ConsumerId; no test defines whether this is queue-share or double-delivery (drain would deliver per match-entry, sharing one inflight budget — `drain.rs:665-721`). Proposed: `same_durable_name_two_connections_semantics` — pin the intended behavior. Rank: 8. |
...
| F1 | Stale-config rejection (CreateConsumer with different config ⇒ error, not silent merge) | **GAP** | Wire path exists (`Ok(2)` → `InvalidConsumerConfig`, `dispatch_v2.rs:1729-1732`) but zero e2e coverage (`grep InvalidConsumerConfig tests/` → none). Proposed: `create_consumer_config_mismatch_rejected` — create with max_inflight 10, re-create same name with 20, assert error and original config still in force. Rank: 6. |
...
7. **Bound slow-consumer backpressure on the delivery path (P1).** After N consecutive
   `Backpressured` cycles (or T ms) for the same conn, mark it dead like WriterGone
   (`src/shard/drain.rs:394-411`), and replace the 50 µs stall-sleep with exponential backoff
   (`src/shard/worker.rs:277-284`).
...
10. **Small hardening batch (P2):** cap command-log replay entry length
    (`src/persistence/command_log.rs:124,143`); clear `dlq_nack_counts` in
    `handle_delete_consumer` (`src/shard/handlers.rs:708-743`); bound `ArbitroStateMachine::applied`
    (`src/cluster/state_machine.rs:72-74`); `out.clear()` on `read_entries` error
    (`src/cluster/storage.rs:824-845`); add e2e for config-mismatch rejection and fsync-every
    kill -9 durability.
