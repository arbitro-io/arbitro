...
impl Message {
    ...
    pub(crate) fn new(
        ...
        Self { seq, consumer_id, stream_id, subject_hash, subject, reply_to, payload, ack_tx, nack_tx }
    }
    ...
    #[inline]
    pub fn reply_to(&self) -> &[u8] { &self.reply_to }

    /// Returns `true` if this message has a reply_to subject (is an RPC request).
    #[inline]
    pub fn has_reply_to(&self) -> bool { !self.reply_to.is_empty() }

    /// Clone the payload `Bytes` handle (zero-copy — bumps an Arc ref-count).
    #[inline]
    pub fn payload(&self) -> Bytes { self.payload.clone() }
