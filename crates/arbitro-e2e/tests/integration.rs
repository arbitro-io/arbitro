...
async fn start_server() -> String {
    ...
    let config = Config::default().listen_addr(addr.clone()).max_connections(256);
...
async fn connect(addr: &str) -> Client {
    Client::connect(ClientConfig { addr: addr.to_string(), ..ClientConfig::default() })
        .await.expect("client must connect")
}
...
async fn test_fanout_delivery() {
    ...
    for i in 0..3u32 {
        ...
        let resp = client.create_consumer(sid, name.as_bytes(), group.as_bytes(), b"", u16::MAX, 0, 0, 0, 0, 0).await.unwrap();
        let cid = parse_id(&resp);
    ...
    for i in 0..10u32 {
        ...
        client.publish(sid, b"fanout.evt", Bytes::copy_from_slice(payload.as_bytes())).expect("publish");
    }

    for (idx, sub) in subs.iter_mut().enumerate() {
        for j in 0..10 {
            assert!(sub.recv().await.is_some(), "consumer {idx} should receive msg {j}");
        }
    }
}
...
async fn test_nack_redelivery() {
    ...
    client.publish(sid, b"nack.msg", Bytes::copy_from_slice(b"data")).expect("publish");
    let msg = sub.recv().await.unwrap();
    msg.nack();
    assert!(sub.recv().await.is_some(), "nacked message should be re-delivered");
}
...
async fn test_replay_publish_then_subscribe() {
    ...
    let resp = client.create_consumer(sid, b"replayer", b"", b"", u16::MAX, 0, 0, 0, 0, 0).await.unwrap();
    let cid = parse_id(&resp);
    let mut sub = client.subscribe(sid, cid, b"").await.unwrap();
    ...
    for i in 0..500 {
        let msg = sub.recv().await.unwrap();
        assert!(msg.seq > prev_seq, "msg {i}: seqs must be monotonic: {} <= {}", msg.seq, prev_seq);
        prev_seq = msg.seq;
    }
}
...
async fn test_gate_auto_delivery_smoke() {
    ...
    for round in 0..3u32 {
        for i in 0..5u32 {
            ...
            client.publish(sid, b"gate.evt", Bytes::copy_from_slice(payload.as_bytes())).expect("publish");
        }
        for _ in 0..5 {
            let msg = sub.recv().await.unwrap();
            msg.ack();
...
async fn test_fanout_same_connection() {
    ...
    for i in 0..3u32 {
        ...
        let resp = client.create_consumer(sid, name.as_bytes(), group.as_bytes(), b"", 100, 1, 0, 0, 0, 0).await.unwrap();
        let cid = parse_id(&resp);
        subs.push(client.subscribe(sid, cid, b"").await.unwrap());
    }

    for i in 0..10u32 {
        client.publish(sid, b"fsc.evt", Bytes::copy_from_slice(&i.to_le_bytes())).expect("publish");
    }
...
async fn test_queue_same_connection() {
    ...
    let resp = client.create_consumer(sid, b"qsc-w2", b"", b"", 100, 1, 0, 0, 0, 0).await.unwrap();
    ...
    for i in 0..10u32 {
        client.publish(sid, b"qsc.job", Bytes::copy_from_slice(&i.to_le_bytes())).expect("publish");
    }
...
async fn test_fanout_filtered_same_connection() {
    ...
    let resp = client.create_consumer(sid, b"ffsc-b", b"gb", b"", 100, 1, 0, 0, 0, 0).await.unwrap();
    let cid_b = parse_id(&resp);
    let mut sub_b = client.subscribe(sid, cid_b, b"payments.*").await.unwrap();

    let resp = client.create_consumer(sid, b"ffsc-c", b"gc", b"", 100, 1, 0, 0, 0, 0).await.unwrap();
    let cid_c = parse_id(&resp);
    let mut sub_c = client.subscribe(sid, cid_c, b">").await.unwrap();

    for i in 0..3u32 {
        let subj = format!("orders.{i}");
        client.publish(sid, subj.as_bytes(), Bytes::copy_from_slice(b"o")).expect("publish");
    }
    for i in 0..2u32 {
        let subj = format!("payments.{i}");
        client.publish(sid, subj.as_bytes(), Bytes::copy_from_slice(b"p")).expect("publish");
    }

    let mut ca = 0u32;
    loop {
        match tokio::time::timeout(Duration::from_secs(2), sub_a.recv()).await {
            Ok(Some(msg)) => { msg.ack(); ca += 1; }
            _ => break,
        }
    }
...
async fn test_publish_with_reply_delivers_reply_to() {
    ...
    client.publish_with_reply(sid, b"rpc.request", reply_subject, Bytes::from_static(b"hello"))
        .await.unwrap();
...
async fn test_publish_without_reply_has_empty_reply_to() {
    ...
    client.publish(sid, b"norpc.msg", Bytes::from_static(b"data")).expect("publish");
