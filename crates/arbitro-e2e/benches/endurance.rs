...
async fn main() {
    ...
    tokio::spawn(async move {
        let _ = server.run_with_shutdown(stop_rx).await;
    });
    ...
    while start_time.elapsed() < TEST_DURATION {
        ...
        let delta_ticks =
            (now_stats.utime + now_stats.stime).saturating_sub(last_stats.utime + last_stats.stime);
        ...
        println!(
            "[{:.0}s] Throughput: {:.2} msg/s | CPU: {:.1}% | RAM: {:.1} MB",
    ...
    println!(
        "Avg Throughput: {:.2} msg/s",
        final_msgs as f64 / final_elapsed
    );
