...
async fn t12_stream_recreation_does_not_cross_contaminate() {
    ...
    for i in 0u32..5 {
        client
            .publish_sync(old_stream_id, b"t12.ev", Bytes::copy_from_slice(&i.to_le_bytes()))
            .await
            .expect("publish");
    }
    ...
    while let Ok(Some(m)) = tokio::time::timeout(Duration::from_millis(150), old_handle.recv()).await {
        m.ack();
    }
    ...
    for i in 100u32..103 {
        client
            .publish_sync(new_stream_id, b"t12.new", Bytes::copy_from_slice(&i.to_le_bytes()))
            .await
            .expect("publish");
    }
    ...
    let resp = client
        .create_consumer(new_stream_id, b"t12_fresh", b"", b"", 100, 1, 0, 0, 30_000, 0)
        .await
        .unwrap();
