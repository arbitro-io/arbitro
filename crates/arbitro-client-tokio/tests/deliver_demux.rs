...
async fn wildcard_subject_fanout_correct() {
    ...
    // ── set up stream ────────────────────────────────────────────────────────
    let resp = client
        .create_stream(b"deliver-test", b"deliver.test", 0, 0, 0, 1, 0, 0, 0, 0)
        .await
...
async fn two_consumers_independent_streams_no_crosstalk() {
    ...
    // ── Stream A + consumer A ─────────────────────────────────────────────────
    let resp_sa = client
        .create_stream(b"demux-stream-a", b"a.subj", 0, 0, 0, 1, 0, 0, 0, 0)
        .await
    ...
    // ── Stream B + consumer B ─────────────────────────────────────────────────
    let resp_sb = client
        .create_stream(b"demux-stream-b", b"b.subj", 0, 0, 0, 1, 0, 0, 0, 0)
        .await
