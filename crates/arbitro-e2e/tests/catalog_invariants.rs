...
// ═══════════════════════════════════════════════════════════════════════════
// GAP-5: Fanout (deliver_mode=0) + non-empty group is an incompatible pair.
// A fanout consumer has no queue, so the group can never load-balance — the
// old dispatch SILENTLY dropped the group and delivered fanout, so a caller
// who set `.group("workers")` but left deliver_mode at its Fanout default
// believed they had a queue and instead got a full-copy fanout. The broker
// must reject it with InvalidConsumerConfig, not swallow it.
//
// Two controls prove the guard is narrow: fanout WITHOUT a group still works,
// and queue-mode (deliver_mode=1) WITH a group still works.
// ═══════════════════════════════════════════════════════════════════════════

#[tokio::test(flavor = "multi_thread")]
async fn fanout_with_group_rejected() {
    use arbitro_client_tokio::ClientError;
    use arbitro_proto::error::ErrorCode;
    ...
    let stream_id = create_stream(&client, b"gap5").await;

    // The incompatible pair: deliver_mode=0 (Fanout) + group="workers".
    let err = client
        .create_consumer(
            stream_id, b"c_bad", b"workers", b"", 100u16, 1u8, 0u8, 0u8, 0u32, 0u64,
        )
        .await
        .expect_err("Fanout + non-empty group must be rejected");
    assert!(
        matches!(
            err,
            ClientError::Broker {
        ...
        "expected InvalidConsumerConfig for Fanout+group, got {err:?}"
    );

    // The rejected create must not have leaked a slot — nothing listed.
    let resp = client.list_consumers(stream_id, 0, 1000).await.unwrap();
    assert_eq!(
        TestServer::consumer_count(&resp),
        0,
        "a rejected Fanout+group create must not leak a consumer entry"
    );

    // Control 1: Fanout WITHOUT a group is still valid.
    client
        .create_consumer(
            stream_id, b"c_fanout", b"", b"", 100u16, 1u8, 0u8, 0u8, 0u32, 0u64,
        )
        .await
        .expect("Fanout with no group must still succeed");

    // Control 2: Queue mode (deliver_mode=1) WITH a group is still valid.
    client
        .create_consumer(
            stream_id, b"c_queue", b"workers", b"", 100u16, 1u8, 0u8, 1u8, 0u32, 0u64,
        )
        .await
        .expect("Queue mode with a group must still succeed");

    let resp = client.list_consumers(stream_id, 0, 1000).await.unwrap();
    assert_eq!(
        TestServer::consumer_count(&resp),
        2,
        "exactly the two valid consumers must exist after the guard test"
    );
    server.shutdown().await;
}
