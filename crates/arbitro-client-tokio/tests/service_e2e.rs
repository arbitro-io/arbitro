...
///
/// The previous version picked a port by binding, DROPPING the listener,
/// and returning the number — then slept 80ms hoping the server had bound
/// it. Both halves were races. Under the full suite the machine is loaded,
/// 80ms was not enough, and the client hit ConnectionRefused; the freed
/// port could also be claimed by a sibling test in the same binary.
